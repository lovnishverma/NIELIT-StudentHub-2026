...
The system now pre-calculates the following score for every project:

> **Score = (2 × Upvotes + 3 × Comments) / √(DaysOld + 1)**

**New Architecture:**
* **Worker:** A function `updateTrendingCache` runs every **1 hour** (via Time-Driven Trigger). It performs the heavy math above and writes the Top 5 results to a dedicated static sheet (`TrendingCache`).
